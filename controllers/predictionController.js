...
async function predictImage(req, res) {
  try {
    ...
    const classes = ['kardus', 'kaca', 'logam', 'organik', 'kertas', 'plastik'];
    ...
    if (error.message.includes('expected input_1 to have shape [null,224,224,3] but got array with shape [1,224,224,4]')) {
      res.status(400).send('Invalid image format: expected an image with 3 color channels (RGB), but got an image with 4 channels (RGBA). Please upload an RGB image.');
    } else {
